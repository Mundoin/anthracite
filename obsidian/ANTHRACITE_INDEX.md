...
| V1AT  | landed · Live Collection Safety Gate + Dry-Run Plan — Pure Rust `live_collection_plan` engine + `plan_live_topology_collection_cmd` Tauri command + TS wire mirror + `LiveCollectionDryRunPanel`. Deterministic dry-run plan: platform/source kinds → read-only commands, planned import mode (merge default), parser route metadata, closed warning set, safety checklist, honesty note. Unsupported (FortiOS/MikroTik) and driver-deferred (Huawei VRP/Nokia SR OS) are honest. No SSH, no credentials, no polling, no scheduler, no evidence-store mutation, no graph renderer. | [stages/V1AT-live-collection-safety-dry-run.md](./stages/V1AT-live-collection-safety-dry-run.md) |
| V1AU  | landed · Fixture-backed Live Collection Simulator — Frontend-only bridge that proves the V1AT → V1AP/V1AQ → V1AR → V1AS flow without any device contact. New `liveCollectionSimulator.ts` + synthetic fixtures (iosxe/nxos/eos × LLDP+CDP, junos LLDP, iosxr LLDP) feed a ready V1AT plan through existing `importTopologyNeighborOutput`. Unsupported/deferred platforms have no fixtures; not-ready plans cannot simulate. No SSH, no credentials, no Rust changes, no new Tauri command, no parser/V1AR semantic changes. | [stages/V1AU-fixture-backed-live-collection-simulator.md](./stages/V1AU-fixture-backed-live-collection-simulator.md) |
| V1AV  | landed · Missing Parser Coverage (Huawei VRP + FortiOS) — New `huawei_vrp` and `fortinet_fortios` parser modules at `PARSER_VERSION = 1`: Huawei covers identity, platform metadata, interfaces (description + admin state + vrf binding), ip addressing (dotted mask + prefix form), static routes, telnet service note, honest out-of-scope trail. FortiOS covers identity, platform metadata, interfaces, VLANs, firewall zones, static routes, note-only interface metadata, and an honest out-of-scope trail. Dispatch wired in `parsers/mod.rs`. Fixture corpora + corpus harnesses mirror the existing per-vendor pattern. IOS-XR and MikroTik remain missing — follow-on stages. No DeviceModel/framework/validator/frontend changes. | [stages/V1AV-missing-parser-coverage.md](./stages/V1AV-missing-parser-coverage.md) |
| V1AW  | landed · Diagnose Seed / Operator Answers v1 — Pure-frontend deterministic answer surface. New `diagnoseProjection.ts` rules over existing Discovery + Topology data: telnet enabled, missing hostname, unknown interface admin state, described-but-no-IP interfaces, unsupported platform (iosxr/mikrotik), out-of-scope parser evidence, topology rejection/no-edge/no-source honesty. `DiagnoseMode.tsx` mounts in mode rail (flipped `not_connected` → `built`); summary strip, ranked answer cards, click-to-inspector with evidence + suggested target + source label, honest empty + clean states. No Rust changes, no Tauri command, no new wire types, no DeviceModel expansion, no validator change, no live collection. | [stages/V1AW-diagnose-seed.md](./stages/V1AW-diagnose-seed.md) |
| V1AX  | landed · Discovery Foundation v1 — Operator-triggered read-only discovery. New `discovery_runner.rs` engine + three Tauri commands (validate/plan/attempt). Target profile form (host/port/username/platform_hint/transport/data_source_label) feeds deterministic, platform-specific read-only command plan. `attempt_discovery_run` honestly defers until SSH transport lands in V1AY. No credentials persisted, no scheduler, no graph renderer. Evidence will flow through V1AP/V1AQ/V1AR into V1AS/V1AW when transport ships. | [stages/V1AX-discovery-foundation-v1.md](./stages/V1AX-discovery-foundation-v1.md) |
| V1AY  | landed · Core Graph Renderer v1 — Pure deterministic SVG renderer. New `buildRenderGraph` adapter (V1AS `GraphReadyTopologyView` → coordinates), `TopologyGraphSurface` (2D SVG widget with click selection), `TopologyGraphInspector` (selected node/edge details), `RenderGraphSourceBadge` (five-value honest data-source badge), `TopologyGraphPanel` (parent composer, selection state reset on model change). SVG chosen for jsdom testability + no-GL honesty. Babylon.js reserved in deps for V1AZ (3D variant). Full type safety (RenderGraphNode, RenderGraphEdge, RenderGraphModel, RenderGraphState, RenderGraphDataSource, RenderGraphSelection). Determinism invariant: same input always yields same layout. No physics, no randomness, no animation, no Babylon imports. Integration adds Graph section to TopologyMode below V1AS review surface; two surfaces read same topology data independently. | [stages/V1AY-core-graph-renderer-v1.md](./stages/V1AY-core-graph-renderer-v1.md) |
| V1AZ  | landed · SSH Transport v1 — Pure-Rust `russh` client (async, no openssl-sys). Operator-triggered read-only execution. Session-only credentials (password or pasted PEM key), forbidden-token gate (`configure terminal`, `write memory`, etc.), timeout/output limits enforced. Five new `DiscoveryRunOutcome` variants: `Captured`, `AuthFailed`, `ConnectionFailed`, `Timeout`, `CommandFailed`. Evidence does not auto-import — operator reviews captured outputs and explicitly runs V1AP/V1AQ import step. No credential persistence, no scheduler, no server-key pinning (V1BA). SshTransport trait + RealRusshTransport + MockTransport for tests. Full coverage: `cargo test` 12+ passing, `pnpm test` 6+ passing, SecretString Debug/Display print `***`, outcomes carry no secret bytes. DiscoveryMode gains credential tab (password/key toggle) + outcome panels. | [stages/V1AZ-ssh-transport-v1.md](./stages/V1AZ-ssh-transport-v1.md) |

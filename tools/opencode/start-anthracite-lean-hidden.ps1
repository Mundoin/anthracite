...
if ($null -ne $portInUse) {
    Write-Host "Lean server already running on port $Port"
...
$cmd = "Remove-Item Env:OPENCODE_CONFIG -ErrorAction SilentlyContinue; Remove-Item Env:OPENCODE_CONFIG_DIR -ErrorAction SilentlyContinue; Set-Location 'D:\Repos\anthracite'; opencode serve --hostname 127.0.0.1 --port $Port *>> '$LogFile'"
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    if ($proc.HasExited) {
        Write-Host "ERROR: server process exited before port $Port was bound"
...
try {
    ...
    if ($health.healthy -eq $true) {
        Write-Host "Lean server started (PID $($proc.Id)) on http://127.0.0.1:$Port"
...
"[$ts] Started lean server PID=$($proc.Id) port=$Port" | Add-Content "$ServiceDir\lean-start.log"

//! NX-OS LAG helpers — V1U.

use crate::engines::network_model::LagMode;

/// NX-OS `channel-group N mode active|passive|on` — parse `(N, mode)`.
...
/// NX-OS LAG interface name form: `port-channel{id}` (lowercase).
...
mod tests {
    ...
    #[test]
    fn lag_name_is_lowercase() {
        assert_eq!(lag_name(1), "port-channel1");
        assert_eq!(lag_name(10), "port-channel10");
    }
}

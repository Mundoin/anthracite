...
const FIXTURE_ROOT: &str = "tests/fixtures/vyos";
...
fn platform_ref() -> PlatformRef {
    PlatformRef {
        ...
        vendor: Some("VyOS".to_string()),
        os_family: Some("VyOS".to_string()),
        os_version_raw: None,
...
fn manifest_parser_version_matches_source() {
    let m = read_manifest();
    assert_eq!(m.parser_version, anthracite_lib::engines::parsers::vyos::PARSER_VERSION);
}

//! V1U cisco-nxos fixture corpus harness.
//!
//! Walks every fixture listed in
//! `tests/fixtures/cisco-nxos/_manifest.toml` and asserts model +
//! serialised-JSON stability against each fixture's committed
//! `expected.json`. Also asserts manifest ↔ on-disk consistency and
//! that the receipt projection round-trips over an NX-OS `DeviceModel`.
//!
//! Seed / re-capture every fixture's `expected.json` by running:
//!     ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test cisco_nxos_fixture_corpus
...
fn parse_fixture(name: &str) -> DeviceModel {
    ...
    parsers::parse_device_config(nxos_platform_ref(), &cfg)
...
fn manifest_parser_version_matches_source() {
    ...
    assert_eq!(
        ...
        "manifest parser_version must equal cisco_nxos::PARSER_VERSION"
    );
}
    ...
    for name in &m.fixtures {
        ...
        if !updating {
            assert!(
                ...
                "fixture {name}: missing expected.json — run `ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test cisco_nxos_fixture_corpus` to bootstrap"
            );
        }
    }
}
...
fn every_fixture_parses_to_committed_expected_json() {
    ...
    assert!(
        mismatches.is_empty(),
        "NX-OS fixture corpus mismatches:\n  {}",
        mismatches.join("\n  ")
    );
}
...
#[test]
fn receipt_projection_round_trips_over_nxos_model() {
    ...
    assert_eq!(r1.platform_id.as_deref(), Some("cisco-nxos"));
}

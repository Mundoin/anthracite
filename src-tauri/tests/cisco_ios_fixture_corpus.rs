//! V1BC cisco-ios fixture corpus harness.
//!
//! Walks every fixture listed in `tests/fixtures/cisco-ios/_manifest.toml`
...
const FIXTURE_ROOT: &str = "tests/fixtures/cisco-ios";
...
/// pulling in a TOML crate (no new deps).
...
fn ios_platform_ref() -> PlatformRef {
    PlatformRef {
...
fn parse_fixture(name: &str) -> DeviceModel {
    ...
    parsers::parse_device_config(ios_platform_ref(), &cfg)
...
fn manifest_parser_version_matches_source() {
    ...
    assert_eq!(
        ...
        "manifest parser_version must equal cisco_ios::PARSER_VERSION"
    );
}
...
fn every_listed_fixture_has_config_and_expected_files() {
    ...
    for name in &m.fixtures {
        ...
        if !updating {
            ...
            assert!(
                exp.exists(),
                "fixture {name}: missing expected.json — run `ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test cisco_ios_fixture_corpus` to bootstrap"
            );
        }
    }
}
...
fn corpus_matches_expected_json_and_is_deterministic() {
    ...
    for name in &m.fixtures {
        ...
        if updating {
            ...
            assert_eq!(
                rendered, expected,
                "fixture {name}: expected.json drifted from parser output"
            );
        }

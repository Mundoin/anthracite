//! V1BA mikrotik-routeros fixture corpus harness.
//!
//! Walks every fixture listed in
//! `tests/fixtures/mikrotik-routeros/_manifest.toml` and asserts model +
...
const FIXTURE_ROOT: &str = "tests/fixtures/mikrotik-routeros";
...
fn parse_fixture(name: &str) -> DeviceModel {
    ...
    parsers::parse_device_config(mikrotik_platform_ref(), &cfg)
...
fn manifest_parser_version_matches_source() {
    ...
    assert_eq!(
        ...
        "manifest parser_version must equal mikrotik_routeros::PARSER_VERSION"
    );
}
    ...
    for name in &m.fixtures {
        ...
        if !updating {
            assert!(
                ...
                "fixture {name}: missing expected.json — run `ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test mikrotik_routeros_fixture_corpus` to bootstrap"
            );
        }
    }
}
...
fn every_fixture_parses_to_committed_expected_json() {
    ...
    assert!(
        mismatches.is_empty(),
        "MikroTik RouterOS fixture corpus mismatches:\n  {}",
        mismatches.join("\n  ")
    );
}

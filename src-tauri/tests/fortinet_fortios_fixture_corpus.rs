//! V1AV fortinet-fortios fixture corpus harness.
//!
//! Walks every fixture listed in `tests/fixtures/fortinet-fortios/_manifest.toml`
//! and asserts model + serialised-JSON stability against each fixture's
//! committed `expected.json`. Also asserts manifest ↔ on-disk consistency and
...
const FIXTURE_ROOT: &str = "tests/fixtures/fortinet-fortios";
...
fn parse_fixture(name: &str) -> DeviceModel {
    ...
    parsers::parse_device_config(fortios_platform_ref(), &cfg)
...
fn manifest_parser_version_matches_source() {
    ...
    assert_eq!(
        ...
        "manifest parser_version must equal fortinet_fortios::PARSER_VERSION"
    );
}
    ...
    for name in &m.fixtures {
        ...
        if !updating {
            assert!(
                ...
                "fixture {name}: missing expected.json — run `ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test fortinet_fortios_fixture_corpus` to bootstrap"
            );
        }
    }
}
...
fn every_fixture_parses_to_committed_expected_json() {
    ...
    assert!(
        mismatches.is_empty(),
        "FortiOS fixture corpus mismatches:\n  {}",
        mismatches.join("\n  ")
    );
}

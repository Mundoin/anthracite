//! Palo Alto PAN-OS fixture corpus harness.
//!
//! Walks every fixture listed in `tests/fixtures/paloalto-panos/_manifest.toml`
...
//!     ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test paloalto_panos_fixture_corpus
...
const FIXTURE_ROOT: &str = "tests/fixtures/paloalto-panos";
...
fn panos_platform_ref() -> PlatformRef {
    PlatformRef {
        platform_id: Some("paloalto-panos".to_string()),
        vendor: Some("Palo Alto Networks".to_string()),
        os_family: Some("PAN-OS".to_string()),
        os_version_raw: None,
...
fn parse_fixture(name: &str) -> DeviceModel {
    ...
    parsers::parse_device_config(panos_platform_ref(), &cfg).expect("parse_device_config Ok")
}
...
fn manifest_parser_version_matches_source() {
    ...
    assert_eq!(
        ...
        "manifest parser_version must equal paloalto_panos::PARSER_VERSION"
    );
}
...
fn every_listed_fixture_has_config_and_expected_files() {
    ...
    for name in &m.fixtures {
        ...
        if !updating {
            ...
            assert!(
                exp.exists(),
                "fixture {name}: missing expected.json — run `ANTHRACITE_UPDATE_FIXTURES=1 cargo test --test paloalto_panos_fixture_corpus` to bootstrap"
            );
        }
    }
}
...
#[test]
fn corpus_matches_expected_json_and_is_deterministic() {
    let m = read_manifest();
    let updating = std::env::var("ANTHRACITE_UPDATE_FIXTURES").is_ok();
    for name in &m.fixtures {
        ...
        if updating {
            write_expected(name, &rendered);
        } else {
            let expected = read_expected(name);
            if rendered != expected {
                let rendered_lines: Vec<&str> = rendered.lines().collect();
                let expected_lines: Vec<&str> = expected.lines().collect();
                let max = rendered_lines.len().max(expected_lines.len());
                for idx in 0..max {
                    let a = rendered_lines.get(idx).copied().unwrap_or("<missing>");
                    let b = expected_lines.get(idx).copied().unwrap_or("<missing>");
                    if a != b {
                        panic!(
                            "fixture {name}: first diff at line {}:\nLEFT : {}\nRIGHT: {}",
                            idx + 1,
                            a,
                            b
                        );
        ...
        assert_eq!(
            rendered, rerendered,
            "fixture {name}: parser output changed across two consecutive parses"
        );
    }
}

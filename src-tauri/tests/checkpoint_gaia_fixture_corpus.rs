//! Check Point Gaia fixture corpus harness.
...
const FIXTURE_ROOT: &str = "tests/fixtures/checkpoint-gaia";
...
fn platform_ref() -> PlatformRef {
    PlatformRef {
        ...
        vendor: Some("Check Point".to_string()),
        os_family: Some("Gaia".to_string()),
        os_version_raw: None,
...
fn parse_fixture(name: &str) -> DeviceModel {
    parsers::parse_device_config(platform_ref(), &read_config(name)).expect("parse_device_config Ok")
}

#[test]
fn manifest_parser_version_matches_source() {
    let m = read_manifest();
    assert_eq!(m.parser_version, anthracite_lib::engines::parsers::checkpoint_gaia::PARSER_VERSION);
}
...
fn every_listed_fixture_has_config_and_expected_files() {
    ...
    for name in &m.fixtures {
        ...
        assert!(d.join("config.cfg").exists());
        if !updating {
            assert!(d.join("expected.json").exists());
        }
    }
}

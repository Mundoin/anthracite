# live-collection-output-to-raw-import-017 intent

This pack prepares OCC for output to raw import in V1AT.
It stays prep-only, synthetic, and sanitised.

## Pack focus
- Show the handoff from raw live output into the existing V1AP raw import path.

## Safe fields
- command_allowlist
- operator_initiated
- dry_run_preview
- raw_output
- source_label
- source_kind
- exact_inventory_match
- raw_output
- import_target

## Note-only areas
- management address exact-match notes only if the inventory schema supports it
- capability hints stay as evidence notes
- redaction notes stay separate from raw text
- unsupported or deferred platforms stay conservative
- parser input is the raw text, not the preview

## Rejection reasons
- connection_failed
- auth_failed
- timeout
- command_unsupported
- unsupported_platform
- deferred_platform
- malformed_output
- empty_output
- no_store_mutation
- handoff_missing

## Raw capture
- OCC may later use this section to keep output to raw import read-only and review-gated.
- Safe fields: command_allowlist, operator_initiated, dry_run_preview, raw_output.
- Conservative behaviour: connection_failed, auth_failed, timeout, command_unsupported stay as no-op or reject outcomes.

## Handoff to V1AP
- OCC may later use this section to keep output to raw import read-only and review-gated.
- Safe fields: command_allowlist, operator_initiated, dry_run_preview, raw_output.
- Conservative behaviour: connection_failed, auth_failed, timeout, command_unsupported stay as no-op or reject outcomes.

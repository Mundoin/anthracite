# 011-missing-local-interface

Purpose: prep-only corpus for V1AS topology edge review and graph-ready surface behaviour.

Scenario: The evidence lacks a local interface, so exact projection stays unsafe.
...
- Missing local interface stays visible as a rejection case.

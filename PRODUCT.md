...
> conflicts with the source of truth, the source of truth wins. See also the
> mode/engine map and the industrial visual law under `docs/`.

## Mission

Anthracite is a **living network intelligence surface**. A local-first desktop
application that gives network engineers a coherent operating surface for
understanding, operating, and reasoning about their networks in real time.

HOME / Environment opens the V1 surface. Topology remains important, but the
source-of-truth doctrine wins over the old "topology is the crown jewel"
framing.

## Persona

Senior network engineer / NetOps lead. Multi-vendor environments (initial
target: 7 vendors). Spends their day staring at CLI, vendor consoles, Grafana,
and disconnected scripts. Wants one coherent surface that breathes with the
network.

## Value Pillars

1. **Living topology.** Information topology and live topology, both 2D and 3D
   selectable, with animated link state. Not a static diagram.
...
4. **Forge.** Authoring + automation surface. Turn surface insight into
...
- Dark industrial theme. Charcoal / anthracite base; restrained accent usage.
...
| Stage | Name                          | Output |
|-------|-------------------------------|--------|
| V1A   | Ground Zero Repo Factory      | Empty surface, scaffold, tooling. **← current** |
